...
| `onAdditionalNodes` | Specifies what to do if the hierarchy in Azure DevOps contains additional nodes (nodes that are not defined by the hierarchy). Possible values: `warning`, `delete`, `keep` and `ignore`. With `keep` the syncronized Test Cases are removed from the node, while `ignore` completely ignores the node, so Test Case duplicates are possible within the hiearchy. | `ignore` for `single` hierarchy type, `warning` for all others |
| `onAdditionalTestCases` | Specifies what to do if the hierarchy nodes in Azure DevOps contain additional Test Cases (test cases that were not synchronized by SpecSync). Possible values: `warning`, `keep` and `remove`. With `warning` (default) a warning is displayed but test cases are kept. With `keep` the additional test cases remain in the node without warning. With `remove` the additional test cases are removed from the node. | `warning` |
