...
| `linkOnChange/links[]/linkTemplate` | <p>Specifies the HTTP link template of the related artifact (for `GitHub Pull Request` relationship). The link template can use the specified value using the `{id}` placeholder.</p> | no template used |
