...
There are multiple installation options you can choose from depending on your operating system and the development platform \(see details at [Installation & Setup](../installation/) page\). In this guide we configure SpecSync for a .NET 10 project and will install SpecSync as a [local .NET Core Tool](../installation/dotnet-core-tool.md).
...
The easiest way to do this is to execute the SpecFlow scenarios as you would do normally and use the [publish-test-results command](../reference/command-line-reference/publish-test-results-command.md) of SpecSync to publish these results to Azure DevOps. 
