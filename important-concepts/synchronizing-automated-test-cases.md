...
To be able to publish the test results to the appropriate test cases it is recommended to perform a synchronization before executing the tests and publishing the results using the SpecSync `push` command. See [Usage](../usage.md) and [Synchronizing test cases from build](synchronizing-test-cases-from-build.md) for details.
